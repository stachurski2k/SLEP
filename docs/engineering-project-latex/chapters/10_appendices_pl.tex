...
%% Rozdział 10: Dodatki
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% UWAGA: plik klasy nie udostępnia polecenia \chapter, a jego makro
%% dodatku \dodatek ma wpisane na stałe polskie słowo "Dodatek" w nagłówku,
%% co byłoby niespójne z pracą napisaną w całości po angielsku. Zamiast niego
%% używamy standardowego mechanizmu \appendix dostępnego w klasie (który
%% jedynie zmienia \thesection na litery A, B, C, ... bez tekstu wpisanego
%% na stałe), więc każdy poniższy dodatek jest sekcją \section oznaczoną literą.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\appendix
\label{chap:appendices}

\section{Przykłady formatowania w szablonie LaTeX}
\label{app:template-reference}

W tym dodatku zachowano przykłady formatowania dołączone pierwotnie do
uczelnianego szablonu klasy \texttt{projektInzynierskiMS1z}. Przeniesiono je tutaj,
zamiast usuwać, aby przykłady podziału na sekcje, równań oraz twierdzeń i definicji
pozostały dostępne jako wzorzec podczas pisania pozostałej części pracy.

\subsection{Przykład podziału na sekcje i numeracji}
\label{app:template-sectioning}

Szablon przedstawia zastosowanie środowisk \texttt{tw} (twierdzenie)
i \texttt{dd} (definicja) oraz automatycznych liczników sekcji i podsekcji:
...
Bieżące wartości liczników: \thesection, \thesubsection, \thesubsubsection.

\subsection{Przykład równania}
...
\subsection{Przykład tekstu wypełniającego}
\label{app:template-filler}

Generator Lorem Ipsum (\texttt{http://lipsum.pl/index.php});
...
\subsection{Przykład drugiej sekcji}
...
\todo{DO UZUPEŁNIENIA: Ta sekcja dodatku pierwotnie zawierała w szablonie
informację „Tu rysunki”; należy przenieść tutaj wszystkie rysunki uzupełniające,
których nie zamieszczono bezpośrednio w treści
rozdziałów~\ref{chap:annotation-platform}--\ref{chap:educational-platform}.}

\section{Listingi programów}
\label{app:programs}
...
Kod źródłowy platformy do anotacji
(rozdział~\ref{chap:annotation-platform}) oraz kod badawczy dotyczący rozpoznawania
i tłumaczenia (rozdziały~\ref{chap:isolated-gloss-recognition}--\ref{chap:sentence-level-translation})
są dostępne w publicznym repozytorium projektu w serwisie GitHub:
