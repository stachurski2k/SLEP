...
const tableActionHeaderCellClass = `${tableHeaderCellClass} w-16 text-right`
...
const selectedRowClass =
  'bg-emerald-300/10 [&>td:first-child]:shadow-[inset_3px_0_0_#3dd9b3]'
...
const tableHeadingClass =
  'mt-2 mb-0 text-[1.35rem] font-semibold tracking-normal text-[#f5f7fb]'
const tableStateTitleClass =
  'mt-2.5 mb-2 text-[1.05rem] font-semibold tracking-normal text-current'
...
export default function CustomTable<Row>({
  ...
  if (isLoading) {
    return (
      <>
        <TableHeader heading={tableHeading} statusLabel="Loading" />
  ...
  if (rows.length === 0) {
    return (
      <>
        <TableHeader heading={tableHeading} statusLabel={`0 ${normalizedLabel}`} />
        <TableState
        ...
        <table className="w-full min-w-[780px] border-collapse">
          <thead>
            <tr>
              ...
              {actions.length > 0 ? (
                <th
                  className={tableActionHeaderCellClass}
                  scope="col"
                  aria-label="Actions"
                />
              ) : null}
...
function TableHeader({ heading, statusLabel }: TableHeaderProps) {
  return (
    <header className="flex items-start justify-between gap-[18px] border-b border-slate-400/10 px-[22px] py-5 max-[720px]:grid">
      <h2 className={tableHeadingClass}>{heading}</h2>
  ...
  return (
    <div className="grid min-h-[220px] place-items-center p-9 text-center text-[#a8b0c3]">
      <div className="max-w-[460px]">
        ...
        <h3 className={tableStateTitleClass}>{title}</h3>
